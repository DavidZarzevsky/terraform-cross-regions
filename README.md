...
| src_region_ami                 | Source instance AMI                                | string       | "ami-02675d30b814d1daa" |
|dst_region_ami                  | Destination instance AMI                           | string       | "ami-01973462ceff35e85" |
...
2. Download AWS Client VPN for Desktop [here](https://aws.amazon.com/vpn/client-vpn-download/).
...
5. SSH to the source account EC2 instance using its private IP.

  To SSH into the EC2 destination account (another account, another region):
  Run the following command:

   ```sh 
   scp -i /your/path/to/key-file /your/path/to/key-file ${EC2 OS name}@${source EC2 private IP}:~/
   ```
   Then SSH into the source EC2 instance. From the first EC2, SSH into the destination EC2 using the key file you copied.
...
This project uses Terraform's capabilities to illustrate how to architect, deploy, and manage multi-account, multi-region infrastructure in the cloud.
